...
namespace LuceneSearch.Service {
	public static class GoLucene {
		...
		// search methods
		public static IEnumerable<Data> GetAllIndexRecords() {
			// validate search index
			if (!System.IO.Directory.EnumerateFiles(_luceneDir).Any()) return new List<Data>();
      ...
      // v 3.0.3: use 'term.Doc'
		...
		public static IEnumerable<Data> Search(string input, string fieldName = "") {
			if (string.IsNullOrEmpty(input)) return new List<Data>();
		...
		public static IEnumerable<Data> SearchDefault(string input, string fieldName = "") {
			return string.IsNullOrEmpty(input) ? new List<Data>() : _search(input, fieldName);
		}

		// main search method
		private static IEnumerable<Data> _search(string searchQuery, string searchField = "") {
			// validation
			if (string.IsNullOrEmpty(searchQuery.Replace("*", "").Replace("?", ""))) return new List<Data>();
		...
		private static IEnumerable<Data> _mapLuceneToDataList(IEnumerable<ScoreDoc> hits, IndexSearcher searcher) {
      // v 2.9.4: use 'hit.doc'
      // v 3.0.3: use 'hit.Doc'
