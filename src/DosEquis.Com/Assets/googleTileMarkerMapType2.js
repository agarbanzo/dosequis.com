...
/** @constructor */
var TileMarkerMapType = function( map, tileMarkerFun ) {
    this.map = map;
...
//    this.tileSize = new google.maps.Size(1024, 1024);
    ...
    var that=this;
//    window.setTimeout(function(){ that.mapMoving = false; }, 6000);
    map.addListener('drag', function() { that.mapMoving = true; }); // check this too because we are waiting a little before seting off
    map.addListener('dragstart', function() { that.mapMoving = true; });
    // wait a little before seting off
    //map.addListener('dragend', function() { window.setTimeout( function() { that.mapMoving = false; } , 1000 ) });
    map.addListener('idle', function() { window.setTimeout( function() { that.mapMoving = false; } , 1000 ) });
}

TileMarkerMapType.prototype.getTile = function (coord, zoom, ownerDocument) {
    //console.log('coord:', coord);
    var latLng = this.coord2LatLng(coord, zoom);
...
TileMarkerMapType.prototype.waitForVisibleTile = function ( tile, latLng ) {
    ...
    var thisCallback = function() {
        // check that the tile is visible
...
//        console.log('tileVisible: ',tileVisible);
        if (tileVisible == 'true') {
            ...
            if (that.isMapMoving()) {
                ...
                if (that.tileMarkersCompleted[tileMarkerKey] !== true) { // prevent it from creating markers again for the same tile
                    that.tileMarkersCompleted[tileMarkerKey] = true;
                    that.tileMarkerFun(latLng.center, latLng.radiusMiles, latLng.NW, latLng.SE);
                }
            }
        }        
    }
...
module.export.TileMarkerMapType =  TileMarkerMapType();